...
class WA_BaseModel(nn.Module):
    ...
    def forward(self, HR, LR, use_HR=False, use_MSE=0, reverse=False):
        ...
        # layer_output_list, last_state_list = self.convGRU(te_output_list)
        '''GRU'''
        te_output_list = te_output_list.view(batch_size, window_size, -1)
        layer_output_list, last_state_list = self.gru(te_output_list)
        '''concat'''
        # convGRU_output = last_state_list[0][0]'''concat'''
        convGRU_output = last_state_list[0][0]
        # convGRU_output = convGRU_output.view(-1, self.out_features)
