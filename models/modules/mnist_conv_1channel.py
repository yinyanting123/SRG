...
class Net(nn.Module):
    def __init__(self, node_num, in_features):
        super(Net, self).__init__()
        self.net = spconv.SparseSequential(
            nn.BatchNorm1d(node_num),
            spconv.SubMConv2d(node_num, in_features, 3, 1),
            nn.ReLU(),
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x: [N, 28, 28, 1], must be NHWC tensor
        channel = 1
        batch_size, window_size, H, W = x.size()[0: 4]
        x = x.reshape(batch_size, window_size, channel, H, W)
        output_list = []
        for b in range(batch_size):
            ...
            output = output.view(1, window_size, channel, H, -1)
            if b == 0:
                output_list = output
            else:
