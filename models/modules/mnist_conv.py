...
class Net(nn.Module):
    def __init__(self, node_num, in_features, hidden_features=64):
        super(Net, self).__init__()
        self.net = spconv.SparseSequential(
            nn.BatchNorm1d(node_num),
            spconv.SubMConv2d(node_num, hidden_features, 3, 1),
            nn.GELU(),
            spconv.SubMConv2d(hidden_features, in_features, 3, 1),
            nn.GELU(),
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x: [N, 28, 28, 1], must be NHWC tensor
        batch_size, window_size, channel, node_num = x.size()[0: 4]
        # x = x.reshape(batch_size, window_size, channel, node_num, node_num)
        output_list = []
        for b in range(batch_size):
            x_sp = spconv.SparseConvTensor.from_dense(x[b])
            # create SparseConvTensor manually: see SparseConvTensor.from_dense
            output = self.net(x_sp)
            output = output.view(1, window_size, channel, node_num, -1)
            if b == 0:
                output_list = output
            else:
